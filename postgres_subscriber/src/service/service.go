...
func (s *Service) SubscribeMessage(f func(msg *nats.Msg)) error {
	_, err := s.NatsConn.Subscribe("test-subject", f)
	return err
}
...
// PostgresSubscriber PostgresSubscriber
func (s *Service) PostgresSubscriber() {
	f := func(msg *nats.Msg) {
		nms := NatsMessageStruct{}
		json.Unmarshal(msg.Data, &nms)

		if err := s.InsertPostgres(nms.ID, nms.Timestamp); err != nil {
			fmt.Println("Postgres Insert error")
			fmt.Println(err)
		}
	}
	if err := s.SubscribeMessage(f); err != nil {
		fmt.Println("Nats Subscribe error")
		fmt.Println(err)
	}

	fmt.Println("Nats Subscribe start")
